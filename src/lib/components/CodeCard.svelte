...
<div>
  <div class="bg-slate-700 dark:bg-zinc-900 rounded-lg overflow-x-auto border">
    <div
      class="flex justify-between items-center px-4 py-2 border-b border-zinc-800 bg-slate-900/50 dark:bg-zinc-950/50"
    >
    ...
    <pre class="m-0 p-0"><code class="font-mono text-sm language-{language} p-0"
        >{code}</code
      ></pre>
    {#if output.length > 0}
      <div class="border-t border-zinc-800">
        <div
          class="text-xs text-zinc-400 bg-slate-900/50 dark:bg-zinc-950/50 px-4 py-2 border-b"
        >
          Output:
        </div>
        <div class="flex flex-col gap-0 py-2">
          {#each output as item}
            <div class="px-4 py-1 text-sm">
              <span class="dark:text-green-600 text-green-400">davidref</span>
              <span class="text-white">></span>

              <span class="text-zinc-300"> {@html item}</span>
            </div>
          {/each}
        </div>
      </div>
